## vix.py
## vix.py is a python script that calculates the CBOE Volatility Index (VIX) 
##inputs
rates=[0.000305, 0.000286] 				
datafiles=['./near.csv', './next.csv']
verbose=0						#0 for just the result, 1 for some output, 2 for more output

import math
import pandas as pd
#get option data from near.csv and next.csv for options expiring on near term and next term expiration dates
quotedata=pd.DataFrame()
for j in (0,1):
    f = pd.read_csv(path, header = 0, sep = ',', quotechar = '"', error_bad_lines = False)
	quotedata[j].append(f)
